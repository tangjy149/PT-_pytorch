...
def index_points(points, idx):
    ...
    B*2048*3，idex=[4,5,6]
    return [B,4,5,6,C]=>每个batch中由4，5，6点组成的B*3*3点集
    """
    raw_size = idx.size()
...
def farthest_point_sample(xyz, npoint):
    ...
    device = xyz.device
    B, N, C = xyz.shape
    # 初始化centroids存储矩阵[B,npoint]=》每个batch有npoint个采样点
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device)
    # 记录batch中每个点（N个）到某个点之间的距离，初始距离较大
    distance = torch.ones(B, N).to(device) * 1e10
    # 随机初始化选取0-N范围中一个点，作为初始最远点，选B次
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    Return:
        ...
        new_points: sampled points data, [B, npoint, nsample, 3+D]——D为特征
    """
    B, N, C = xyz.shape
    S = npoint
    fps_idx = farthest_point_sample(xyz, npoint)  # [B, npoint]
    torch.cuda.empty_cache()
    new_xyz = index_points(xyz, fps_idx)  # centroid
    torch.cuda.empty_cache()
    if knn:
        # knn直接获取前k个点的索引 [B,npoint,k]
        dists = square_distance(new_xyz, xyz)  # B x npoint x N
        idx = dists.argsort()[:, :, :nsample]  # B x npoint x K
    else:
        # 以radius半径取nsample个采样点，的索引 [B,npoint,nsample]
    ...
    if points is not None:
        ...
        new_points = torch.cat([grouped_xyz_norm, grouped_points], dim=-1)  # [B, npoint, nsample, C+D] 或 [B,npoint,K,C+D]
    else:
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, knn=False):
        """
        初始化参数
        :param npoint: 采样点（centroid）
        :param radius: 划分区域半径
        :param nsample: 区域内点个数
        :param in_channel: 输入通道数
        :param mlp: [MLP各层的通道输出大小]
        :param group_all:
        :param knn:
        """
    ...
    def forward(self, xyz, points):
        ...
        # 1*1 2d卷积 将每一个group当作一个通道，共npoint
